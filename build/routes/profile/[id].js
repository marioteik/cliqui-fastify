...
const PUT = async (req, rep) => {
    if (!req.headers.authorization) {
        return rep.status(401).send({ message: "Unauthorized" });
    }
    const supabase = (0, supabase_1.getSupabase)(req.headers.authorization);
    const { data: user, error: errorFindUser, status: statusFindUser, } = await supabase.from("profiles").select().eq("id", req.params.id);
    if (!user || (user && !user[0])) {
        const { data, error, status } = await supabase
            .from("profiles")
            .insert(Object.assign(Object.assign({}, req.body), { id: req.params.id }))
            .select()
            .single();
        if (error) {
            rep.status(status).send(error);
